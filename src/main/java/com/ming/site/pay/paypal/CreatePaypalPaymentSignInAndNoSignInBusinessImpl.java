...
@Service
public class CreatePaypalPaymentSignInAndNoSignInBusinessImpl
        implements CreatePayment {
    private static final Logger log = LoggerFactory.getLogger(CreatePaypalPaymentSignInAndNoSignInBusinessImpl.class);
    @Autowired
    ...
    public Payment create(Order order) {
        ...
        var isNewOrder = order.getId() == null || order.getId() <= 0;
        if (isNewOrder) {
